(ns advertising-marketing.actor
  "AdvertisingMarketingActor — the ISCO-08 2431 independent
  advertising-and-marketing actor as a `langgraph.graph/state-graph`
  (per ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  campaign operation request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the CampaignAdvisor can never directly
  commit a record or dispatch a robot action the
  AdvertisingMarketingGovernor refuses — every commit-record! call is
  gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [advertising-marketing.advisor :as advisor]
            [advertising-marketing.governor :as governor]
            [advertising-marketing.store :as store]))

(defn build-graph
  "Build a compiled AdvertisingMarketingActor graph. `store`
  implements `advertising-marketing.store/Store`. `advisor` implements
  `advertising-marketing.advisor/Advisor` (defaults to
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:campaign-id (:campaign-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
